  ...
  return (
    <div className="rounded-lg border border-[#e8ecf0] bg-white p-6 shadow-sm sm:p-8">
      ...
      <div className="mb-6 flex items-center gap-3">
        ...
        <span className="text-xs text-[#8b97a5]">{step === 1 ? "Your Details" : "Vessel Details"}</span>
      </div>
      ...
      {step === 1 && (
        <form onSubmit={handleStep1} noValidate className="space-y-4">
          ...
          <button
            type="submit"
            disabled={!form.name || !form.email || !form.yachtType}
            className="btn-gold w-full py-4 disabled:opacity-50"
          >
            Continue →
          </button>
          <p className="text-center text-xs text-[#8b97a5]">
            All submissions are reviewed privately. No public exposure without your explicit consent.
          </p>
        </form>
      )}
      ...
      {step === 2 && (
        <form onSubmit={handleSubmit} noValidate className="space-y-4">
          ...
          <div className="flex gap-3">
            ...
            >
              {loading ? "Submitting…" : "Submit for Confidential Review"}
            </button>
          </div>
